...
SHEET_ID = "1VqhzrV1n4qySGxBENXl0csxZYrpLRyaDlREMa0r04Ss"
...
# 6. SECURE CLIENT DASHBOARD
    ...
    if not recs_df.empty:
        ...
        published_recs = recs_df[recs_df["status"].astype(str).str.strip().str.lower() == "published"]
        
        if not published_recs.empty:
            unique_matches = published_recs["match_name"].unique()
            st.markdown(f"✨ 系統狀態：🟢 目前已順利解鎖 **{len(unique_matches)}** 場核心賽事")
            st.write("") 
            
            for match_name in unique_matches:
                match_data = published_recs[published_recs["match_name"] == match_name]
                
                with st.expander(f"⚽  {match_name}", expanded=False):
                    st.write("")
                        ...
                        st.markdown(f"""
                            ...
                            <div style="background: rgba(15, 23, 42, 0.4); padding: 16px 14px 4px 14px; border-left: 1px solid rgba(255,255,255,0.03); border-right: 1px solid rgba(255,255,255,0.03);">
                                <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;">
                                    ...
                                    <span class="rec-pill">{rec_title}</span>
                                </div>
                                <div style="font-size: 0.9rem; font-weight: 700; color: #38BDF8; display: flex; align-items: center; gap: 4px; margin-bottom: 6px;">
                                    🗣️ 雨姐短評 (Commentary)：
                                </div>
                            </div>
